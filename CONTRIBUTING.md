# Contributing to ep_loading_message

Thanks for helping improve `ep_loading_message` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_loading_message` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_loading_message from your local clone
pnpm run plugins i --path ../ep_loading_message

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_loading_message/` are picked up after a server restart.
...
* Etherpad version, ep_loading_message version, browser + OS
* Steps to reproduce
